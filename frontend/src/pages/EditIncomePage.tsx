...
import { GET_INCOME } from "../graphql/queries/income";
import IncomeForm from "../components/income/IncomeForm";

export default function EditIncomePage() {
  const { id } = useParams<{ id: string }>();
  const navigate = useNavigate();
  const { data, loading, error } = useQuery(GET_INCOME, {
    variables: { id },
    skip: !id,
  });
    ...
    return (
      <div className="animate-fade-up max-w-xl">
        ...
        <div className="flex flex-col items-center justify-center py-16">
          ...
          <p className="text-lg font-medium text-[var(--text-primary)] mb-1">Income not found</p>
          <p className="text-sm text-[var(--text-muted)]">The income stream you're looking for doesn't exist</p>
        </div>
      </div>
    );
  }
